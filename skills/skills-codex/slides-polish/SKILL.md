...
- **REVIEWER_MODEL = `gpt-5.6-sol`** — Codex model for per-page review. xhigh reasoning is non-negotiable (see `../shared-references/effort-contract.md`). If the account has no `gpt-5.6-sol` access, return `REVIEW_UNAVAILABLE`; use another model only when the user explicitly overrides the reviewer route.
- **REVIEWER_REASONING = `xhigh`** — Hard invariant; the effort knob does **not** change this.
- **CONTEXT_POLICY = `fresh`** — Each per-page review uses a **fresh** Codex reviewer call (`spawn_agent`, never `followup_task`). See `../shared-references/reviewer-independence.md`. This prevents the reviewer from anchoring on prior fixes.
...
6. **Fresh-context independence**: per-page Codex calls are fresh `spawn_agent` calls, not `followup_task`. Reviewer never sees prior fix lists; base verdicts are same-family provisional.
